...
void getRadixSelectWorkSpaceSize(const int& K, const int& n, const int& taskNum,
  ...
  // TODO: This function is wrong, don't use K now.
  // Find out how sizeInBytes is computed
  *sizeInBytes =
...
// TODO: is it necessary to change type(a) into CompT?
...
#pragma unroll
  for (int i = tx; i < histLen; i += BLOCK_THREADS) {
    // TODO : Optimize it with vec load
    if (blockHist[i] > 0) {
...
// TODO: implement with cub::BlockScan
...
__global__ void __launch_bounds__(1024)
  ...
  const int mask = binIdPtr[taskId];
  const int row_idx = indices == nullptr ? taskId : indices[taskId];
  //   printf("taskId: %d, row_idx: %d\n", taskId, row_idx);
  vec_t<T, VEC_SIZE> dataIn_vec;
  // printf("taskLen: %d, BLOCK_THREADS: %d, VECS_ZE: %d\n", taskLen, BLOCK_THREADS, VEC_SIZE);
  // TODO: handle taskLen % VEC_SIZE != 0
...
#pragma unroll
    ...
    // select top-k elements, it can be wrong if select top-k and element which larger than
    // top - k element simultaneously
#pragma unroll 2
    for (uint32_t i = 0; i < ceil_div(taskLen, BLOCK_THREADS * VEC_SIZE); i++) {
      if ((i * BLOCK_THREADS + tx) * VEC_SIZE < taskLen) {
        dataIn_vec.cast_load(dataIn + row_idx * stride + (i * BLOCK_THREADS + tx) * VEC_SIZE);
...
template <int BLOCK_THREADS, typename IdxType, typename ValType>
__global__ void __launch_bounds__(1024)
    FillWithTopKIndicesKernel(const ValType* dataIn,
                              const typename ComputeT<ValType>::type* kThElePtr, IdxType* idxOut,
                              const int* taskLenPtr, const int stride, IdxType* indices) {
  using CompT = typename ComputeT<ValType>::type;
  ...
  for (int i = tx; i < taskLen; i += BLOCK_THREADS) {
    if (dataIn[row_idx * stride + i] == kThElem) {
      idxOut[taskId] = i;
      break;
    }
  }
...
#pragma unroll
  ...
  //   if (tx == 0) {
  //     printf("row_idx: %d, batch_id: %d, block_sum: %f, radik-renorm normalizer: %f\n", row_idx,
  //     bx,
  //            block_sum, renorm_factor);
  //   }
...
workSpace 起始地址
│
├─ valBuffer[0] ────────────────────── CompT类型，大小：taskNum × d
│   (第一个值缓冲区)
│
├─ valBuffer[1] ────────────────────── CompT类型，大小：taskNum × d
│   (第二个值缓冲区)
│
├─ histPtr ─────────────────────────── int类型，大小：(1 << 12) × taskNum = 4096 × taskNum
│   (直方图缓冲区)                        (每个task有4096个bin的直方图)
│
├─ globalCountPtr ──────────────────── int类型，大小：taskNum
│   (全局计数器)
│
├─ taskLenPtr[0] ───────────────────── int类型，大小：taskNum
│   (旧任务长度数组)
│
├─ taskLenPtr[1] ───────────────────── int类型，大小：taskNum
│   (新任务长度数组)
│
├─ kPtr ────────────────────────────── int类型，大小：taskNum
│   (K值数组)
│
└─ binIdPtr ────────────────────────── int类型，大小：taskNum
    (bin ID数组)
*/
...
cudaError_t RadiKSamplingFromProb(T* probs, IdType* output, IdType* indices, int* top_k_arr,
                                  ...
                                  uint64_t philox_offset, void* workspace_buffer,
                                  size_t workspace_buffer_size_in_bytes, T* selected_probs,
                                  cudaStream_t stream = 0) {
    ...
    DISPATCH_ALIGNED_VEC_SIZE(vec_size, VEC_SIZE, {
      ...
      if (maxTaskLen != 1) {
        ...
        FLASHINFER_CUDA_CALL(cudaLaunchKernel((void*)selectbin_iter2_kernel, selectbin_iter2_nblks,
                                              selectbin_iter2_nthrs, selectbin_iter2_args,
                                              smem_size, stream));
      ...
      if (maxTaskLen != 1) {
        ...
        FLASHINFER_CUDA_CALL(cudaLaunchKernel((void*)selectbin_iter3_kernel, selectbin_iter3_nblks,
                                              selectbin_iter3_nthrs, selectbin_iter3_args,
                                              smem_size, stream));
...
#define RADIX_TOPK_CALL_FILTER(CACHE_SIZE)                                                  \
  do {                                                                                      \
    ...
    FLASHINFER_CUDA_CALL(cudaLaunchKernel((void*)filter_kernel, filter_nblks, filter_nthrs, \
      ...
      //   dim3 fill_nblks(output_batch_size);
      //   dim3 fill_nthrs(BLOCK_THREADS);
      //   auto fill_kernel = FillWithTopKIndicesKernel<BLOCK_THREADS, IdType, T>;
      //   void* fill_args[] = {&probs, &valBuffer[flag], &output, &taskLenPtr[0], &d, &indices};

      //   FLASHINFER_CUDA_CALL(cudaLaunchKernel((void*)fill_kernel, fill_nblks, fill_nthrs,
      //   fill_args,
      //                                         0, stream));
    });

    // TODO: Optimize FinalSampling's vec_size
    // TODO : Fix filterKernel for indices
    vec_size = 1;
    // printf("vec_size: %d, top_k_val: %d\n", vec_size, top_k_val);
    // printf("top_k_select_result: %p, top_k_select_idx: %p, output: %p, indices: %p\n",
    //        top_k_select_result, top_k_select_idx, output, indices);
    DISPATCH_ALIGNED_VEC_SIZE(vec_size, VEC_SIZE, {
  ...
  // // 分配CPU内存来存储第一个任务的histogram
  // std::vector<int> host_histogram(1 << 12, 0);  // 4096个元素

  // // // 将第一个任务的histogram从GPU复制到CPU
  // FLASHINFER_CUDA_CALL(
  //     cudaMemcpy(host_histogram.data(), histPtr, sizeof(int) * (1 << 12),
  //     cudaMemcpyDeviceToHost));
  // // 打印第一个任务的histogram
  // printf("=== Task 0 Histogram ===\n");
  // int total_count = 0;
  // int non_zero_bins = 0;
  ...
  //   }
  // }
  // printf("Total count: %d, Non-zero bins: %d\n", total_count, non_zero_bins);

  // std::vector<int> host_binId(batch_size);
  // std::vector<int> host_k(batch_size);
  // std::vector<int> host_taskLen(batch_size);
  ...
  // cudaMemcpy(host_taskLen.data(), taskLenPtr[1], sizeof(int) * batch_size,
  // cudaMemcpyDeviceToHost);

  // // 验证第一个任务的结果
  // for (int task_idx = 0; task_idx < batch_size; task_idx++) {
  //   printf("=== SelectBin Results for Task %d ===\n", task_idx);
  //   printf("Selected bin ID: %d\n", host_binId[task_idx]);
  //   printf("Updated k: %d\n", host_k[task_idx]);
  //   printf("Bin count: %d\n", host_taskLen[task_idx]);
  // }

  // // 添加 globalCount 结果的复制和打印
  // std::vector<int> host_globalCount(batch_size);
  // cudaMemcpy(host_globalCount.data(), globalCountPtr, sizeof(int) * batch_size,
  //            cudaMemcpyDeviceToHost);

  // // 打印 globalCount 结果
  // printf("=== GlobalCount Results ===\n");
